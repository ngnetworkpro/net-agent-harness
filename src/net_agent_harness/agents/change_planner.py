...
def planner_system_prompt(ctx: RunContext[RunContextData]) -> str:
    ...
    preamble = [
        ...
        route_summary,
        "Output requirements: ",
        "Return structured output conforming to the PlannedChange schema: ",
        "- scope: ScopeRef object containing: ",
        "  - site: site name (e.g. HQ) if mentioned or known, or null. ",
        "  - region: region name if mentioned, or null. ",
        "  - device_names: list of explicit device names (e.g., ['sw1', 'fw1']) if they are named/mentioned in the request. Always populate this if explicit device names are mentioned in the request. ",
        "  - device_roles: list of explicit device roles (e.g., ['access switch']), or empty list. Do not invent roles. ",
        "  - requested_role: primary role phrase if the user requests a role-based target, or null. ",
        "- target_scope: 'device', 'site', or 'ambiguous' ",
        "- target_resources: list of ResourceRef objects, each with a resource_type in [site, device, interface, vlan, vrf, subnet, prefix, ip_assignment, topology_link] ",
        "- resource_relationships: list of ResourceRelationship objects, using relationship_type in [site_to_device, interface_to_subnet, device_to_topology_link] when known ",
        "- requested_change: RequestedChange object containing: ",
        "  - summary: short summary of the intended change ",
        "  - intent: plain English requested change ",
        "  - desired_state: containing operations list, grouped by object_type: vlan, interface, svi, etc. ",
        ...
        "- assumptions: list of assumptions explicitly ",
        "- dependencies: list of dependencies ",
        "- rollback_plan: RollbackPlan object ",
        "- plan_decision: PlanDecision object if you decide to block/no-op, or null ",
        "missing_information: ",
