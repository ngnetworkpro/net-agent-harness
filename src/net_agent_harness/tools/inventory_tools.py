...
from ..adapters.mock_inventory_adapter import get_mock_inventory_snapshot
...
def _normalize_device(item: dict) -> dict:
    ...
    return {
        ...
        'status': (item.get('status') or {}).get('value') or (item.get('status') or {}).get('label'),
        'role': (item.get('role') or {}).get('name'),
        'platform': (item.get('platform') or {}).get('name') if item.get('platform') else None,
...
def _normalize_interface(item: dict) -> dict:
    return {
        'id': item.get('id'),
        'name': item.get('name'),
        'type': (item.get('type') or {}).get('label') if isinstance(item.get('type'), dict) else item.get('type'),
        'enabled': item.get('enabled'),
        'mtu': item.get('mtu'),
        'description': item.get('description'),
        'mode': (item.get('mode') or {}).get('value') if isinstance(item.get('mode'), dict) else item.get('mode'),
        'untagged_vlan': ((item.get('untagged_vlan') or {}).get('vid') if isinstance(item.get('untagged_vlan'), dict) else None),
        'tagged_vlans': [v.get('vid') for v in item.get('tagged_vlans', []) if isinstance(v, dict)],
    }
...
def _normalize_ip(item: dict) -> dict:
    ...
    return {
        ...
        'family': (item.get('family') or {}).get('value') if isinstance(item.get('family'), dict) else item.get('family'),
        'status': (item.get('status') or {}).get('value') or (item.get('status') or {}).get('label'),
...
def lookup_inventory(ctx: RunContext[RunContextData], site: str | None = None, device_name: str | None = None) -> dict:
    ...
    if source == 'netbox':
        ...
        return {'source': 'netbox', 'count': payload.get('count', len(devices)), 'results': devices}

    return get_mock_inventory_snapshot(site=site, device_name=device_name)


def lookup_device_context(ctx: RunContext[RunContextData], site: str | None = None, device_name: str | None = None) -> dict:
    source = (ctx.deps.inventory_source or 'mock').lower()
    if source != 'netbox':
        snapshot = get_mock_inventory_snapshot(site=site, device_name=device_name)
    ...
    if not payload.get('results'):
        return {'source': 'netbox', 'device': None, 'interfaces': [], 'ip_addresses': []}
