...
class TerraformBackendAdapter(BackendAdapter):
    ...
    def apply(self, config_render: ConfigRender) -> ExecutionResult:
        ...
        pr_url = self._create_github_pr(updated_content, branch_name)
    ...
    def _create_github_pr(self, updated_content: str, branch_name: str) -> str:
        ...
        with httpx.Client(headers=headers, base_url="https://api.github.com") as client:
            # Get base branch SHA
            ref_resp = client.get(f"/repos/{repo}/git/ref/heads/{settings.github_base_branch}")
            if ref_resp.status_code != 200:
                raise RuntimeError(f"GitHub API error {ref_resp.status_code}: {ref_resp.text}")
            ...
            if branch_resp.status_code not in (201, 422):  # 422 = branch already exists
                raise RuntimeError(f"GitHub API error {branch_resp.status_code}: {branch_resp.text}")

            # Get current file SHA
            file_resp = client.get(f"/repos/{repo}/contents/networks.json", params={"ref": branch_name})
            ...
            put_resp = client.put(f"/repos/{repo}/contents/networks.json", json=payload)
            ...
            if pr_resp.status_code != 201:
                raise RuntimeError(f"GitHub API error {pr_resp.status_code}: {pr_resp.text}")
